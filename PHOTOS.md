...
| `about.jpg` | Vertical photo of you in the About section. Any vertical crop works — it's shown at whatever shape the file is, never cropped. If you swap it for a differently-shaped file, update the `width`/`height` on that `<img>` in `index.html` to match, or the page will jump as it loads. |
